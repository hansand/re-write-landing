...
description: Restrained white canvas, one plum accent, flat-vector illustration of the product mechanism itself.
colors:
  brand-400: "#B4419A"
  brand-500: "#963681"
  brand-600: "#792C68"
...
**Key Characteristics:**
- One plum accent on a white-and-near-black neutral system; color marks meaning, not mood
...
- Three deliberate register shifts: white (default) → near-black (Privacy) → solid plum (final CTA) → near-black (footer)

## Colors

Almost entirely neutral (white, near-black ink, one warm-gray muted, one cool-gray surface) with a single plum family carrying every accent, CTA, and "this is the AI part" signal.

### Primary
- **Plum 500** (`#963681`, `brand-500`): the compose-bubble fill, focus rings, selected-state highlights, and small accent marks (checkmarks, toggles, active icons). Never used as a large background field — it stays a mark, not a ground.
- **Plum 600** (`#792C68`, `brand-600`): default background for every primary button and the Final CTA section's solid ground. Both 500 and 600 clear AA for white body text (~6.7:1 and ~8.9:1 respectively), but 600 stays the button/CTA ground — a deeper, more deliberate fill than the lighter mark tone, with extra contrast margin to spare.
- **Plum 700** (`#5A214D`, `brand-700`): primary-button hover state only.
- **Plum 400** (`#B4419A`, `brand-400`): the lighter stop in the compose-bubble's gradient fill, and small icon accents on the dark Privacy section (needs to sit on near-black, so it's lifted a step lighter than 500).
...
### Named Rules
**The Mark, Not Ground Rule.** Plum covers small shapes (bubbles, buttons, toggles, checks, a single gradient blob) or a single fully-committed section (Final CTA). It never fills a large neutral region as a tint or wash — the accent's rarity is what makes it read as "this is where the AI is."
...
### Shadow Vocabulary
- **Card hover** (`box-shadow: 0 16px 36px -18px rgba(11,11,15,0.16)`, or `-16px rgba(150,54,129,0.35)` on the featured plum-tinted card): feature-grid cards on hover only; flat at rest.
- **Button rest** (`box-shadow: 0 1px 2px rgba(11,11,15,0.06)`): the barest separation on primary/onBrand buttons.
- **Button hover** (`box-shadow: 0 8px 20px -6px rgba(150,54,129,0.55)`): primary button only, a plum-tinted lift.
...
- **Primary:** `brand-600` fill, white text, barely-there rest shadow, plum-tinted lift on hover, background steps to `brand-700`.
...
- **onBrand:** solid white fill, `brand-600` text — the one button variant meant to sit on the solid-plum Final CTA section.

### Cards / Containers
- **Corner style:** `rounded-2xl` (16px) uniformly.
- **Background:** `surface` (#F8F8FB) at rest; the single "featured" feature-card (Multiple real variants) uses a diagonal `surface`-to-white-tinted-plum gradient wash plus a `brand-500/15` ring instead, marking it as the headline feature without breaking the grid's shape language.
...
Every illustration on the page shares one grammar: a browser-chrome or panel frame (white fill, hairline stroke, soft drop shadow), muted-gray placeholder text rendered as rounded bars rather than real copy, and exactly one plum touchpoint per scene (a bubble, a toggle, a checkmark, a pin) marking where the AI acts. Dashed plum-tinted connector lines (`#F2A6E1`, `stroke-dasharray`) link cause and effect between panels (a toggled profile → the compose window; a saved chat → the compose window). The Privacy illustration is the one variant built for the dark register: near-black panel fills, `brand-400` (not 500) for its shield/lock accent since 500 sits closer to the dark ground's own hue family.

## Do's and Don'ts

### Do:
- **Do** keep plum confined to marks and the one fully-committed Final CTA section — never a background wash on a light section.
- **Do** use `brand-600`, not `brand-500`, wherever white body-weight text sits directly on a solid plum fill; 500 fails AA for normal text at any reasonable opacity.
- **Do** give every shadow real blur and a soft spread; a shadow without blur is a costume, not depth.
- **Do** carry the flat-front vector grammar (frame + hairline + one plum touchpoint + dashed connector) into any new illustration rather than introducing photographic or isometric treatments.
...
- **Don't** introduce a second accent hue. Every "this is the AI" signal in the system is plum; a second color would break the one legible signal the palette is built around.
